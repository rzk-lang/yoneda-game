...
The plan is geometric, and follows the route of the accompanying [exercise sheet](https://github.com/jonweinb/syn-infty-cat/blob/main/exercises/03a-yoneda-lemma.rzk.md). First you prove one lemma about an arbitrary square: its two composites are equal. Then you build a square from a composable pair, push it through `ϕ`, and feed the result to the lemma. The right unit law and path concatenation turn the equality it gives into naturality.
...
*By the end you will be able to:* turn any square in a Segal type into an equality of composites, build the codomain square, transform it by `ϕ`, and assemble the naturality equality.
