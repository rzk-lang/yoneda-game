...
- text: 'At `refl` the homotopy is `\ x → refl`, so that is the base case. Assemble `ind-path` over the function type `(x : X) → A x` with the motive above and this base case, then feed `g` and `p` yourself.'
