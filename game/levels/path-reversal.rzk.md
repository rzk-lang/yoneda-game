...
- text: 'The motive sends an endpoint `b` and a path `x = b` to `b = x`. At `refl` that is `x = x`, so the base case is `refl`. Assemble `ind-path` with that motive and base case, then pass `y` and `p`.'
id: path-reversal
inventory:
- name: ind-path
