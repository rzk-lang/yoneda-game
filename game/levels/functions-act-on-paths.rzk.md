...
- text: 'The motive sends an endpoint `b` and a path `x = b` to the goal `f x = f b`. At `refl` that is `f x = f x`, so the base case is `refl`. Assemble `ind-path` with that motive and base case, then pass `y` and `p`.'
