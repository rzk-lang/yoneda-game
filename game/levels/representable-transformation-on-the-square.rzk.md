...
inventory:
- 'codomain-square : (A : U) (is-segal-A : is-segal A) (a b x y : A) (f : hom A x y) (v : hom A y a) → (t : Δ¹) → (s : Δ¹) → A | the square glued from the two triangles'
statement: '(t : Δ¹) → hom A (f t) b'
title: Transforming the square
---

Now push the square through the transformation. For each `t`, the column `\ s → codomain-square … t s` is an arrow into `a`, so `ϕ` carries it to an arrow into `b`. Apply `ϕ` column by column. The result is a new square whose every column lands in `b`. The `#def` name `square-transformation` is short for `square-representable-transformation`. Build it.
